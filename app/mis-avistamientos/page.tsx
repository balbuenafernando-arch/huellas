...
export default function MySightingsPage() {
  ...
  useEffect(() => { Promise.all([getSightings(), listCases(true), getCurrentUser()]).then(([items, cases, user]) => { setSightings(user ? items.filter((item) => item.owner_token === user.id) : []); setCaseNames(Object.fromEntries(cases.flatMap((item) => [[item.id, item.pet.nombre], ...(item.petId ? [[item.petId, item.pet.nombre]] : [])]))); }).finally(() => setLoading(false)); }, []);
