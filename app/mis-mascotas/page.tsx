...
export default function MisMascotasPage() {
  ...
  async function load() { setPets(await listMyRegisteredPets()); }
  useEffect(() => { load(); }, []);
    ...
    <main className="container py-6">
      ...
      <div className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
        <section className="space-y-3">
          ...
          {pets.map((pet) => <article key={pet.id} className="form-card flex flex-col gap-3 min-[390px]:flex-row">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-[#7A7871]">{pet.especie} · {pet.raza} · {pet.color}</p>
        ...
        {showForm ? <form onSubmit={submit} className="form-card space-y-4">
          ...
          <div className="grid gap-3 md:grid-cols-2"><div><label className="label">Raza</label><input className="field" name="raza" defaultValue={editing?.raza} /></div><div><label className="label">Color</label><input className="field" name="color" required defaultValue={editing?.color} /></div></div>
        ...
        </form> : <aside className="form-card text-sm text-[#6B6860]">Registrar la ficha completa hace más rápido publicar un aviso y validar coincidencias reales.</aside>}
      </div>
    </main>
  );
}
