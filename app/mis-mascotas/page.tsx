...
import { CalendarDays, Edit, Eye, Image as ImageIcon, Plus, Star, Trash2, X } from "lucide-react";
...
export default function MisMascotasPage() {
  ...
  function handlePhoto(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (availableSlots <= 0 || selected.length > availableSlots) {
      setError("Puedes subir hasta un máximo de 3 fotografías por mascota.");
    }
  ...
  function handleCameraPhoto(file: File) {
    const validationMessage = validateImageFiles([file]);
    if (validationMessage) return setError(validationMessage);
    if (formPhotos.length >= 3) return setError("Puedes subir hasta un máximo de 3 fotografías por mascota.");
    setCropFile(file);
    setPendingCropFiles([]);
    setError("");
  }
  ...
  function removeFormPhoto(id: string) {
    if (formPhotos.length <= 1) {
      setError("Si solo existe una fotografía, no puedes eliminarla hasta subir otra o eliminar la mascota.");
      return;
    }
    setFormPhotos((current) => {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (formPhotos.length < 1) errors.fotos = "Debes subir al menos 1 fotografía de la mascota.";
    if (formPhotos.length > 3) errors.fotos = "Puedes subir hasta un máximo de 3 fotografías por mascota.";
  ...
  return (
    <main className="container py-6">
        ...
        <div className="fixed inset-0 z-[1100] grid place-items-center bg-black/60 p-4">
          <section className="form-card max-w-lg space-y-4">
            ...
            <div className="space-y-3 text-sm leading-6 text-[#4D4A43]">
              <p className="font-bold">¿Estás seguro de que deseas eliminar esta mascota?</p>
              <p>Esta acción es permanente y no puede deshacerse.</p>
              <p>Al eliminar esta mascota también se eliminarán:</p>
              <ul className="list-disc pl-5">
                <li>la ficha de la mascota;</li>
                <li>todas sus fotografías;</li>
                <li>la búsqueda asociada (si existe);</li>
                <li>los avistamientos asociados;</li>
                <li>las notificaciones relacionadas.</li>
              </ul>
              <p>Toda esta información se eliminará de forma definitiva y no podrá recuperarse.</p>
            </div>
      ...
      {successMessage && (
        <div className="fixed right-4 top-4 z-[1300] flex max-w-sm items-center gap-3 rounded-xl bg-[#E1F5EE] p-4 text-sm font-semibold text-[#085041] shadow-[0_16px_40px_rgba(0,0,0,.18)]" role="status" aria-live="polite">
          <span>{successMessage}</span>
          <button type="button" className="grid min-h-8 min-w-8 place-items-center rounded-lg hover:bg-black/5" onClick={() => setSuccessMessage("")} aria-label="Cerrar mensaje"><X size={16} /></button>
        </div>
      )}
      ...
      <div className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
        <section className="space-y-3">
          ...
          {pets.map((pet) => {
            const activeCase = cases.find((item) => item.petId === pet.id && item.status !== "reunido" && item.status !== "archivado");
            return (
              <article key={pet.id} className="form-card flex flex-col gap-3 min-[520px]:flex-row">
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className={`rounded-full px-3 py-1 text-xs font-bold ${activeCase ? "bg-[#FDE8E3] text-[#9F2D18]" : "bg-[#E1F5EE] text-[#085041]"}`}>{activeCase ? "🔴 Búsqueda activa" : "🟢 Sin búsqueda activa"}</span>
                  </div>
                  ...
                  <div className="mt-3 grid gap-2 min-[390px]:flex min-[390px]:flex-wrap">
                    ...
                    {activeCase ? <Button size="sm" asChild><Link href={`/pet/${activeCase.id}`}>Ver centro de búsqueda</Link></Button> : <Button size="sm" asChild><Link href={`/perdi-mi-mascota?petId=${pet.id}`}>Reportar pérdida</Link></Button>}
        ...
        {showForm ? (
          <form ref={formRef} onSubmit={submit} className="form-card space-y-4">
            ...
            <div className="rounded-xl bg-[#F8F7F4] p-3 text-sm leading-6 text-[#4D4A43]">
              ...
              Las imágenes deben corresponder únicamente a la mascota registrada. No está permitido subir contenido sexual o con desnudos, violento o extremadamente gráfico, ofensivo o discriminatorio, que promueva odio o acoso, que infrinja derechos de autor o privacidad de terceros, o que no esté relacionado con la mascota. El contenido que incumpla estas normas podrá ser eliminado y la cuenta podrá ser suspendida.
            </div>
            ...
            <div data-field="fotos">
              ...
              <div className="grid gap-2 min-[390px]:grid-cols-2">
                <CameraCapture disabled={saving || formPhotos.length >= 3} onCapture={handleCameraPhoto} />
                <Button type="button" variant="outline" onClick={() => galleryInputRef.current?.click()} disabled={saving || formPhotos.length >= 3}><ImageIcon size={18} />Elegir desde galería</Button>
              </div>
              {formPhotos.length >= 3 && <p className="mt-2 text-sm font-semibold text-[#6B4A10]">Puedes subir hasta un máximo de 3 fotografías por mascota.</p>}
              ...
              <div className="mt-3 grid gap-3 min-[520px]:grid-cols-3">
                {formPhotos.map((photo) => (
                  <div key={photo.id} className="rounded-xl border border-black/10 bg-white p-2">
                    <img src={photo.previewUrl} alt="Fotografía de la mascota" className="h-32 w-full rounded-lg bg-[#F8F7F4] object-contain" />
                    <p className="mt-2 text-xs font-semibold text-[#085041]">{photo.persistedUrl ? "Fotografía guardada" : "Lista para guardar"}</p>
          ...
          <aside className="form-card space-y-4">
            ...
            <dl className="grid gap-3 text-sm min-[390px]:grid-cols-2">
              ...
              <div><dt className="font-bold">Tamaño</dt><dd>{detailPet.tamano === "Pequeno" ? "Pequeño" : detailPet.tamano || "No indicado"}</dd></div>
